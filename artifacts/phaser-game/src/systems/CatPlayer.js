...
// Scale: 128 × 128 source → 64 × 64 display.
const CAT_SCALE = 0.5;
...
export class CatPlayer {
  ...
  constructor(scene, platformGroup, surfaceY) {
    ...
    // ── Sprite ────────────────────────────────────────────────────────────
    // Spawn 1 px above the surface so the body doesn't overlap on frame 0.
    // Gravity snaps it into contact within the first physics step (invisible).
    // Origin (0.5, 1): anchor at bottom-centre → sprite.y == feet position.
    ...
    // Narrow hitbox centred horizontally, flush to the bottom of the frame.
    //   body.bottom = sprite.y (feet) because:
    //   body.y = sprite.y - displayOriginY + offsetY = sprite.y - 64 + 6 = sprite.y - 58
    //   body.bottom = body.y + 58 = sprite.y ✓
    body.setSize(40, 58);
    body.setOffset(12, 6);
